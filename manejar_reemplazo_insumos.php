...
function obtenerCoberturasDisponibles($conexion) {
    $tipo_insumo = 1; // Supongamos que el tipo de insumo para coberturas es 1
    $consulta = $conexion->prepare("
        SELECT insumo.id_insumo, insumo.nombre_insumo 
        FROM insumo 
        INNER JOIN stock ON insumo.id_insumo = stock.id_insumo
        WHERE insumo.tipo_insumo = :tipo_insumo AND stock.porcion > 0
    ");
    $consulta->bindParam(':tipo_insumo', $tipo_insumo, PDO::PARAM_INT);
    $consulta->execute();

    $coberturas = [];
    while ($row = mysqli_fetch_assoc($consulta)) {
        $coberturas[$row['id_cobertura']] = $row['nombre_cobertura'];
    }
    return $coberturas;
}

function obtenerProteinasDisponibles($conexion) {
    $tipo_insumo = 2; // Supongamos que el tipo de insumo para proteínas es 2
    $consulta = $conexion->prepare("
        SELECT insumo.id_insumo, insumo.nombre_insumo 
        FROM insumo 
        INNER JOIN stock ON insumo.id_insumo = stock.id_insumo
    ...
    while ($row = mysqli_fetch_assoc($consulta)) {
        $proteinas[$row['id_proteina']] = $row['nombre_proteina'];
    }
    return $proteinas;
...
function obtenerVegetalesDisponibles($conexion) {
    $tipo_insumo = 3; // Supongamos que el tipo de insumo para vegetales es 3
    $consulta = $conexion->prepare("
        SELECT insumo.id_insumo, insumo.nombre_insumo 
        FROM insumo 
        INNER JOIN stock ON insumo.id_insumo = stock.id_insumo
...
if (isset($_POST['action']) && $_POST['action'] == 'get_reemplazo') {
    ...
    foreach ($insumos_faltantes as $insumo) {
        ...
        } elseif (strpos($insumo, 'vegetal') !== false) {
